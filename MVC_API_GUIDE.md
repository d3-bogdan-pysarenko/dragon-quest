...
Цей проєкт має простий MVC-приклад у папці `src/mvc` і готовий API-шар у папці
`src/api`.
...
Коли користувач натискає кнопку, Controller викликає потрібний метод Model, а
потім оновлює View:
...
Якщо компоненту потрібна тільки поведінка в браузері, але не потрібні
API-запити, створюй для нього окремий TS-файл. Не пиши логіку компонента прямо в
`src/main.ts`.
...
- Якщо потрібен клік, toggle або проста DOM-логіка - створи окремий TS-файл без
  MVC, підключи його в entrypoint і перевіряй у ньому наявність свого `data-*`
  контейнера.
...
Loading і error краще контролювати в Controller, бо він знає, коли запит почався
і коли завершився.
